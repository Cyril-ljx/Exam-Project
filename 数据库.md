...
notice：外部用page列出来，里面先获取id在用index进行下一篇(无上一篇，有时间可实现)

leacot：留言列表(list)+用户留言(insert)

index:用户首页

article:外部用page列出来，里面先获取id在用index进行下一篇(无上一篇，有时间可实现)



## 管理员：

user:增删查改+重置密码（改那里无密码，无增加方法）

updatepassword：改密码，可按照原本单独改，也可直接放在上面user一起改
